...
export const getProfile = asyncHandler(
    async (req, res, next) => {
        const qrcode = await generateQRcode({email: req.user.email, username: req.user.username})
        return res.send({message: 'Done', user: req.user, qrcode})
    }
)
...
export const profilePicture = asyncHandler(
    async (req, res, next) => {
        ...
        const {public_id, secure_url} = await cloudinary.uploader.upload(req.file.path, {
            folder: `Sarahah/Profile/${req.user._id}`,
...
export const forgetPassword = asyncHandler(
    async (req, res, next) => {
        ...
        const token = jwt.sign({id: checkUser._id}, process.env.RESET_PASSWORD_TOKEN_SIGNATURE, {expiresIn: '1d'})
        ...
        sendEmail({to: email, subject: 'Reset Password', html})
...
export const resetPassword = asyncHandler(
    async (req, res, next) => {
        ...
        jwt.verify(token, process.env.RESET_PASSWORD_TOKEN_SIGNATURE, (err, decoded) => {
            if(err){
        ...
        const user = await userModel.updateOne({_id: decodedData.id}, {password: hashPassword})
